...
angular.module('sbAdminApp', ['oc.lazyLoad', 'ui.router', 'ui.bootstrap', 'angular-loading-bar', ]).config(['$stateProvider', '$urlRouterProvider', '$ocLazyLoadProvider', function ($stateProvider, $urlRouterProvider, $ocLazyLoadProvider) {
    ...
    $stateProvider.state('dashboard', {
        ...
        resolve: {
            loadMyFile: function ($ocLazyLoad) {
                return $ocLazyLoad.load({
                    ...
                    $ocLazyLoad.load({
                        name: 'sbAdminApp',
                        files: ['scripts/controllers/chartContoller.js','scripts/directives/chat/chat.js']
                    })
            }
        }
        ...
        url: '/table'
    }).state('dashboard.imdb', {
        templateUrl: 'views/imdb.html',
        controller: 'ImdbCtrl',
        url: '/imdb',
        resolve: {
            loadMyFiles: function ($ocLazyLoad) {
                return $ocLazyLoad.load({
                    name: 'sbAdminApp',
                    files: ['scripts/controllers/imdb.js']
                })
            }
        }
